...
import {
  getReferenceAngles,
  getAngleConfig,
  type TennisTechnique,
  type StrokePhase,
} from "./reference-angles";
...
// compareToReference
//
// Scores extracted forehand angles against evidence-based reference values
// from reference-angles.ts. Returns a 0–100 score per angle and a weighted
// overall score. All reference data, tolerances, and weights live in
// reference-angles.ts — this function only applies the Gaussian scoring model.
// ---------------------------------------------------------------------------
...
export function compareToReference(
  ...
  const reference = getReferenceAngles(technique, phase);
  const config = getAngleConfig(technique);
  ...
  for (const key of keys) {
    ...
    const { tolerance, weight } = config[key];

    // Gaussian falloff: score=100 at deviation=0, ~61 at 1σ, ~14 at 2σ
    const deviation = Math.abs(actual - ref);
    const score = Math.round(100 * Math.exp(-(deviation ** 2) / (2 * tolerance ** 2)));
